...
for(const cat of categories){
 ...
 const body=`<section class="page"><div class="eyebrow">CATEGORY</div><h1>${esc(cat[0].toUpperCase()+cat.slice(1))} tools</h1><p>Browse ${items.length} privacy-first ${esc(cat)} tools. Search is instant and each tool has its own documentation, metadata and related links.</p><input class="tools-search" id="toolSearch" placeholder="Search this category…" aria-label="Search category"><div class="tool-grid">${items.map(t=>`<a class="tool-card" href="${base}tools/${t[0]}/"><span class="eyebrow">${esc(cat)}</span><h3>${esc(t[1])}</h3><p>Private browser-based utility.</p></a>`).join('')}</div></section>`;
