import{spawnSync}from'node:child_process';import{cpSync,mkdirSync,readFileSync,readdirSync,rmSync,writeFileSync}from'node:fs';import{join}from'node:path';
...
const addToolStyles=dir=>{const file=join(toolRoot,dir,'index.html');if(!readFileSync(file,'utf8').includes('tool-page.css')){const html=readFileSync(file,'utf8').replace('</head>','<link rel="stylesheet" href="/tool-page.css">\n</head>');writeFileSync(file,html)}};
